...
**Custom vocabulary + terminology normalization:** a shared one-term-per-line text file (`AppSettings.customVocabularyPath`, security-scoped bookmark via `VocabularyFileAccess`) primes both ASR engines with domain terms, bounded and validated before either consumes it (`CustomVocabularyValidation` in `AppSettings+Vocabulary.swift`: size/term-count/term-length caps, surfaced in Settings → Transcribe).
- **Parakeet** applies it via revision-aware CTC boosting: `ParakeetVocabularyConfiguration` pairs the file path/bookmark with a `WhisperVocabularyPrompt.FileRevision` (fileID + mtime + size) so an edited file is detected without re-reading it on every job, and boosting preparation runs as an optional batch enhancement — preload and live captions stay free of the extra work.
- **WhisperKit** treats it as an opt-in, experimental decoder prompt (`AppSettings.whisperKitVocabularyPromptEnabled`, off by default): `WhisperVocabularyPrompt` converts the term list into a bounded token prompt (`defaultTokenBudget = 32`, keeps the terms that fit within WhisperKit 1.1.0's shared 224-token decoder context) and caches the prepared tokens per model+file-revision (`WhisperVocabularyPrompt.TokenCache`). It's a decoder hint, not a correction, and trades some dense-audio completeness for term recall — disclosed in the Settings help text.
- **Terminology normalization** is a separate, opt-in post-ASR pass (`AppSettings.terminologyRulesText`, `Canonical => spoken variant | another variant` rules, one per line): `TerminologyNormalizer` compiles bounded case-insensitive whole-word/phrase regexes and `PipelineQueue+Stages` applies it to every saved transcript segment. It runs after transcription regardless of which engine or vocabulary path produced the text, so it also cleans up terms WhisperKit/Parakeet get right structurally but spell inconsistently.
...
- Versioned automation API under `/v1` (carries a stability contract, kept off the debug `/action/*` surface): `POST /v1/transcribe` (blocking one-call: 200 terminal / 202 still-running / 400), `POST /v1/jobs` + `GET /v1/jobs/<id>` (enqueue + poll), `GET`/`POST /v1/jobs/<id>/naming` + `POST /v1/jobs/<id>/naming/skip` (speaker naming; 409 on wrong state, 404 unknown id), `GET`/`POST /v1/watch` (meeting-watching lifecycle as an idempotent resource — start/stop/toggle, 503 if the state doesn't settle within 20s) and `GET`/`POST /v1/record` (microphone-only recording lifecycle, same shape; 412 if nothing would be captured — no mic or a denied/broken mic permission — since that is stable until a setting changes, unlike the retryable 503). The two POST enqueue routes honour an `Idempotency-Key` header. Finished-job readback survives the 60s queue reaping + an app restart via `TerminalJobStore`. Full reference: `docs/automation-api.md`. Routing in `DebugRPCServer+V1.swift`.
